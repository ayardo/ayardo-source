...
// which are part of the Ayardo repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="ayardo-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /ayardo-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "ayardopay-ayardo/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from ayardopay-ayardo/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'ayardopay-ayardo/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from ayardopay-ayardo/develop"
            }
          }

          if (hasCache) {
            sh "cd /ayardo-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /ayardo-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /ayardo-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /ayardo-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /ayardo-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /ayardo-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /ayardo-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
