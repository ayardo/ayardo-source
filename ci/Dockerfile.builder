...
# ayardo_hash
RUN git clone https://github.com/ayardopay/ayardo_hash
RUN cd ayardo_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} ayardo
RUN useradd -u ${USER_ID} -g ayardo -s /bin/bash -m -d /ayardo ayardo
...
RUN mkdir /ayardo-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /ayardo-src && \
