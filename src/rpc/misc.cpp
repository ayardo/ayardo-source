...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ayardo balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized ayardo balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "                       mempoolrej, net, proxy, prune, qt, rand, reindex, rpc, selectcoins, tor, zmq, ayardo\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "ayardo")
            + HelpExampleRpc("debug", "ayardo+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given ayardo address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The ayardo address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The ayardo address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Ayardo address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ayardo addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ayardo address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The ayardo address to use for the signature.\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Ayardo features */
    { "ayardo",               "mnsync",                 &mnsync,                 true,  {} },
    { "ayardo",               "spork",                  &spork,                  true,  {"value"} },
