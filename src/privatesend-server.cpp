...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Ayardo related functionality
...
/*
    ...
    allow endless transaction that would bloat Ayardo and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Ayardo specific functionality
