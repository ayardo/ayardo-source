...
const char * const BITCOIN_CONF_FILENAME = "ayardo.conf";
const char * const BITCOIN_PID_FILENAME = "ayardod.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "ayardo" is a composite category enabling all Ayardo-related debug output
                if(ptrCategory->count(std::string("ayardo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ayardo
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ayardo
    // Mac: ~/Library/Application Support/Ayardo
    // Unix: ~/.ayardo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ayardo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ayardo";
#else
    // Unix
    return pathRet / ".ayardo";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ayardo.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ayardo.conf
