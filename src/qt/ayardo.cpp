...
{
    return QCoreApplication::translate("ayardo-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ayardo.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ayardo.qrc)
...
/** Class encapsulating Ayardo Core startup and shutdown.
...
/** Main Ayardo application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // ayardo: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Ayardo Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(ayardo);
    Q_INIT_RESOURCE(ayardo_locale);
    ...
    /// 6. Determine availability of data directory and parse ayardo.conf
    ...
    // ayardo: links repeatedly have their payment requests routed to this process:
